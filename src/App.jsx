...
function App() {
  let [progress,setProgress]=useState(35);
  ...
  return (
    <div className="w-full grid justify-center">
      <div className="w-[350px] md:w-[700px] mt-[100px]">
        <p className="text-3xl font-bold">Flash Cards</p>
        <div className="w-[350px] md:w-[700px] flex justify-between">
          <div className={`mt-[5px] rounded-lg bg-blue-500 text-right h-[40px]`} style={{width: `${window.innerWidth>768?progress:progress/2}px`}}><h4 className="mt-1 mr-3 text-lg text-amber-50">{(progress/700*100).toFixed()}%</h4></div>
        </div>
        <div>
          <div className="w-[350px] md:w-[700px] h-[400px] bg-gray-100 mt-3 rounded-xl border-gray-400 border-2 flex justify-center p-6 items-center">
            <p className={`${showA==false?'font-bold md:text-4xl text-3xl':'font-medium md:text-2xl text-xl'} text-center`}>{showA==false?qs[(progress/700*20)-1]:as[(progress/700*20)-1]}</p>
          </div>
          <div className="flex px-7 py-1 justify-between md:w-[700px] w-[350px] h-[65px] rounded-lg border-solid border-gray-400 border-2 bg-gray-100 mt-2">
            <button id="previousTab" onClick={prevOpt} className="md:text-xl text-lg hover:text-red-500 cursor-pointer active:text-red-600">{`< Previous`}</button>
            <button id="showAns" onClick={()=>showA==false?setA(true):setA(false)} className="md:text-xl text-lg font-medium p-2 m-1 hover:cursor-pointer bg-gray-300 rounded-xl">{showA?"Hide Answer":"Show Answer"}</button>
            <button id="nextTab" onClick={nextOpt} className="md:text-xl text-lg hover:text-green-500 cursor-pointer active:text-green-600">{`Next >`}</button>
          </div>
        </div>
      </div>
    </div>
